...
There was also changes in types int was used for some values but because int removes leading 0s in java I chose to replace this implementation for simplicity this is an assumption I made as there was no reuirments for types within the request this could however break the contract with the vendor so a different solution could be required.
...
- Existing requirements: There was already two separate responses so I assumed this was direction preferred by the code owners.

Service:
This layer was also partially implemented. I started by using constructor based dependancy injection to ensure:
- Immutability
- Thread safety
- Ease of unit testing
- Compliance with SOLID principles
I wanted to keep this layer clear of business logic - there is logic to getAuthorizationCode and UUID. To decouple this and be fully compliant with SOLID principles I would need to create a IdGenerationStrategy however to avoid complexity for a class of this size I decided that keeping it here is acceptable but happy to take guidance against it.


Repository:
The repository is largely unchanged I replaced the previous PostPaymentResponse with a PaymentEntitiy. This is a result of using two different responses the GetPaymentResponse and PostPaymentResponse it is also good practice to decouple the repository layer from the service layer responses.
...
- In case of an error in connecting to the api I log at the error level the status and the body but return an internal server error to the Merchant. This is because I assumed we don't want the merchant being aware of our underlying connections and issues but I would look for guidance on existing practices here.
I also added an error handler to handle logging the exceptions.
